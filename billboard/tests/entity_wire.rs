...
use billboard::entity::{
    ArmorStand, BillboardMode, BlockDisplay, BlockState, DisplayContext, EquipmentSlot, ItemStr,
    PosePart, StandFlags, TextDisplay, TextFlags, WeakMut, WeakRef,
};
...
/// Compile-time contract of the task-safety model, as of the shared-memory
/// engine: weak refs are `Sync` (shareable), and an owner handle is `Send`, so
/// it can be *moved* into a spawned task. It was deliberately `!Sync` while
/// tasks had fork-copied memories — one handle in two memories would have
/// despawned twice — and there is one memory now, so the guard is gone.
#[test]
fn weak_refs_are_sync_and_owners_are_send() {
    fn requires_sync<T: Sync>() {}
    fn requires_send<T: Send>() {}
    requires_sync::<WeakRef<BlockDisplay>>();
    requires_sync::<WeakMut<BlockDisplay>>();
    requires_send::<BlockDisplay>();
    requires_send::<TextDisplay>();
    requires_send::<ArmorStand>();
}
