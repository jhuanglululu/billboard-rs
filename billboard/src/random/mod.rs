...
//! declared `#[billboard::main(random_seed = N)]`.
//!
...
///
/// Its seed defaults to a stable hash of (animation, placement, viewer), so a
/// `per_player` billboard shows each player the same variation on every visit;
/// `#[billboard::main(random_seed = N)]` replaces that seed with `N` and makes
/// [`default_random`] route here.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
...
pub enum DefaultRng {
    ...
    /// `#[billboard::main(random_seed = N)]` was declared.
...
/// Called by `#[billboard::main(random_seed = N)]`; not for animations to call.
#[doc(hidden)]
...
/// per-instance stream if the animation declared
/// `#[billboard::main(random_seed = N)]`.
