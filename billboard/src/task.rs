...
//!
//! The closure is `Sync`-bounded, and entity owner handles are deliberately
//! `!Sync`: capturing one is a *compile error*. That guard is what lets
//! handles stay simple moves (no owner bookkeeping, no runtime checks) —
//! the owner provably never crosses into a child task. Capture
//! [`WeakRef`](crate::entity::WeakRef)/[`WeakMut`](crate::entity::WeakMut)
//! (both `Sync + Clone`) instead.
...
impl Task {
    ...
    ///
    /// The killed task's destructors **never run**, so any entities it owned
    /// are *orphaned*: they stay alive until the animation ends (the host
    /// despawns everything then), and weak references to them keep working.
